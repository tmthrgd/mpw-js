	...
	function pbkdf2_js(password, salt, iter, keyLen, hash) {
		switch ((hash.name || hash).toUpperCase()) {
			...
			case "SHA-512":
				...
				break;
			default:
				var err = new Error("A parameter or an operation is not supported by the underlying object");
		...
		var data = new Uint8Array(salt.length + 4 /*sizeof(uint32)*/);
		...
		return window.crypto.subtle.importKey("raw", password, {
			...
			var _loop = function _loop(block, dki) {
				x = x.then(function () {
					return dataView.setUint32(salt.length, block, false /*big-endian*/);
				}).then(function () {
			...
			for (var block = 1, dki = 0; block <= numBlocks; block++, dki += hashLen) {
				_loop(block, dki);
			}
	...
	if (window.crypto.subtle) {
		return function (password, salt, iter, keyLen, hash) {
			...
			return window.crypto.subtle.importKey("raw", password, {
				name: "PBKDF2",
				hash: hash
			}, false /*not extractable*/, ["deriveBits"]).then(function (key) {
				...
				return(
					...
					: err.name === "InvalidAccessError" ? pbkdf2_js.apply(self, args) : Promise.reject(err)
				);
			});
		};
	} else {
		return function (password, salt, iter, keyLen, hash) {
			...
			return new Promise(function (resolve, reject) {
				...
				window.setImmediate(function () {
					...
					var key = new Uint8Array(Ckey.words.length * 4 /*sizeof(int32)*/);
